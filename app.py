...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    ...
    try:
        while True:
            ...
            if frame_data.get("type") == "update_image":
                # Update user's image for expression
                expression = frame_data.get("expression")
                image_path = frame_data.get("image_path")
                if expression and image_path:
                    user_sessions[client_id]["images"][expression] = image_path
                    await websocket.send_text(json.dumps({"success": True, "message": f"Image updated for {expression}"}))
                continue
...
<body>
    <div class="container">
        ...
        <div class="demo-notice">
            <strong>🎯 Full MEME Tracker Active:</strong> Complete facial expression and hand gesture detection with image selection! 
            Upload images below and see them appear based on your expressions!
        </div>
