...
class FacialLandmarks:
    ...
    def __init__(self):
        ...
        # Smile detection parameters - more conservative to reduce sensitivity
        self.smile_params = {
            'scaleFactor': 1.3,  # Less sensitive scaling
            'minNeighbors': 20,  # Higher threshold to reduce false positives
            'minSize': (40, 40),  # Larger minimum size
            'maxSize': (140, 90)  # Slightly smaller maximum size
        }
    ...
    def detect_smile_simple(self, frame: np.ndarray, face_coords: Tuple[int, int, int, int]) -> Dict:
        ...
        # Focus on mouth area (lower portion of face) - more restrictive
        ...
        smiles = self.smile_cascade.detectMultiScale(gray_mouth_region, **self.smile_params)
        ...
        if len(self.smile_history) >= 3:
            ...
            is_smiling = smile_votes >= 3  # Need at least 3 out of 5 recent detections to be smiling
        else:
    ...
    def _is_eye_open(self, eye_roi: np.ndarray) -> bool:
        ...
        # - There's sufficient brightness variation (not too uniform)
        # - The center isn't much darker than the overall mean
        is_open = (std_brightness > 6) and (center_brightness > mean_brightness - 15)
